...
export default function PosPage() {
  ...
  if (status === 'loading') {
    return <div className="flex h-full items-center justify-center bg-green-50 text-xl font-semibold text-green-800">Memuat Sesi...</div>;
  }
  ...
  return (
    <div className="relative flex h-full w-full overflow-hidden bg-green-50 font-sans">
      <div className="flex w-full flex-col p-4 md:w-3/5 lg:w-2/3">
        <div className="flex-shrink-0">
          <div className="flex items-baseline justify-between">
            ...
            <div className="flex space-x-1 rounded-lg bg-green-200 p-1">
      ...
      <div className={`fixed inset-y-0 right-0 z-40 flex h-full w-full max-w-sm transform flex-col bg-white shadow-2xl transition-transform duration-300 ease-in-out md:relative md:max-w-none md:translate-x-0 lg:w-1/3 ${activeView === 'pos' ? (isCartOpen ? 'translate-x-0' : 'translate-x-full md:translate-x-0') : 'translate-x-full'}`}>
        <div className="flex flex-shrink-0 items-center justify-between border-b p-4 md:hidden">
            <h2 className="text-2xl font-bold text-gray-800">Checkout</h2>
            <button onClick={() => setIsCartOpen(false)} className="rounded-full p-2 text-gray-500 hover:bg-gray-100"><CloseIcon/></button>
        </div>
        <div className="flex h-full flex-col p-6">
            <h2 className="hidden flex-shrink-0 border-b pb-4 text-2xl font-bold text-gray-800 md:block">Checkout</h2>
            <div className="my-4 flex-grow overflow-y-auto pr-2">
              {cart.length === 0 ? (
                <div className="flex h-full flex-col items-center justify-center text-center text-gray-400">
                  <CartIcon/>
                  <p className="mt-4 font-semibold">Keranjang Anda kosong</p>
                </div>
              ) : (
                  ...
                  {cart.map(item => (
                    <li key={item.id} className="flex items-center gap-4 py-3">
                      ...
                      <div className="flex items-center gap-2">
                        <button onClick={() => updateQty(item.id, item.qty - 1)} className="rounded-full p-1.5 transition bg-gray-100 hover:bg-red-100 text-gray-600 hover:text-red-600"><MinusIcon/></button>
                        <span className="w-6 text-center text-sm font-semibold">{item.qty}</span>
                        <button onClick={() => updateQty(item.id, item.qty + 1)} className="rounded-full p-1.5 transition bg-gray-100 hover:bg-green-100 text-gray-600 hover:text-green-600"><PlusIcon/></button>
                      </div>
                       <button onClick={() => updateQty(item.id, 0)} className="text-gray-400 hover:text-red-500 transition-colors"><TrashIcon/></button>
                    </li>
                  ))}
                </ul>
              )}
            </div>
            <div className="flex-shrink-0 space-y-3 border-t-2 border-dashed pt-4">
              ...
              <button 
                ...
                className="w-full rounded-lg bg-green-600 py-4 text-base font-bold text-white shadow-lg shadow-green-200 transition hover:bg-green-700 disabled:cursor-not-allowed disabled:bg-gray-400"
              >
